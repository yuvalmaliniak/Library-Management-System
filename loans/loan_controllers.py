...
class LoanOperations:
    ...
    def __init__(self):
        ...
        self.db = self.client['loans_database']
        self.loan_collection = self.db['loans']
    ...
    def create_loan(self, data):
        ...
        new_loan = {
            ...
            'ISBN': isbn,
            'bookID': book_data[0]['_id'],
            'memberName': memberName,
            'loanDate': loanDate
        }
        result = self.loan_collection.insert_one(new_loan)
        loanID = str(result.inserted_id)
        if not loanID:
            return {"error":"An error occurred while creating the loan"}, 422
        return loanID, 201
    ...
    def get_loan_by_id(self, loan_id):
        loan = self.loan_collection.find_one({'_id': ObjectId(loan_id)})
        if loan:
    ...
    def delete_loan(self, loan_id):
        result = self.loan_collection.delete_one({'_id': ObjectId(loan_id)})
        if result.deleted_count == 1:
            return loan_id, 200
        return None, 404

    def get_from_books(self, query_params=None):
        url = "http://booksrv:8000/books"
